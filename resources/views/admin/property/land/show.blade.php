        ...
        <div class="ld-card">
            <div class="ld-card-head">
                <h6 class="ld-card-head-title">
                    <svg viewBox="0 0 24 24" fill="currentColor" style="width:15px;height:15px;margin-right:6px;color:#64748b">
    ...
    <div class="col-xl-4">
        <div class="position-sticky" style="top:24px">
            ...
            <div class="ld-card">
                ...
                <div class="ld-card-body">
                    @php
                    $payment = $land->payments()->with('listingPackage')->latest()->first();
                    @endphp

                    @if($payment)
                    <div class="ld-plan-item">
                        <span class="ld-plan-label">Package</span>
                        <span class="ld-plan-val">{{ $payment->listingPackage?->package_tier ?? 'N/A' }}</span>
                    </div>
                    <div class="ld-plan-item">
                    ...
                    <div class="ld-plan-item">
                        <span class="ld-plan-label">Reference</span>
                        <span class="ld-plan-val" style="font-family:monospace;font-size:.78rem;color:#C8873A;">
                            {{ $payment->reference }}
                        </span>
                    </div>
                    <div class="ld-plan-item">
                    ...
                    @if($payment->status === 'completed' && !$land->is_approved)
        ...
        <form method="POST" action="{{ route('admin.properties.lands.status', $land) }}" class="modal-content">
            ...
            <div class="modal-header border-0 pb-0">
                <div class="d-flex align-items-center gap-2">
                    ...
                    <h6 class="mb-0">Change land Status</h6>
                </div>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body pt-3">
                <p class="text-muted small mb-2">You are changing the status of the following land:</p>
                <div class="bg-light rounded p-3 mb-3">
                    <p class="fw-600 mb-1">{{ $land->title }}</p>
                    <p class="text-muted small mb-0">
                        Listed by <b>{{ $land->user->name }}</b> · {{ $land->district }}, {{ $land->province }}
                    </p>
                </div>
                <div class="mb-3">
                    ...
                    <select class="form-select" id="statusSelect" name="status" required>
                        ...
                        <option value="rented" {{ $land->status === 'rented' ? 'selected' : '' }}>Rented</option>
                    </select>
                </div>
...
{{-- ══ APPROVE MODAL ══ --}}
@if(isset($payment) && $payment?->status === 'completed' && !$land->is_approved)
