...
<style>
    ...
    .ae-input:focus, .ae-select:focus, .ae-textarea:focus { border-color: var(--accent); box-shadow: 0 0 0 3px #D0520818; }
    ...
    .ae-img-upload:hover { border-color: var(--accent); background: #D0520806; }
    .ae-img-upload input { position: absolute; inset: 0; opacity: 0; cursor: pointer; width: 100%; height: 100%; }
    .ae-img-upload-icon { width: 36px; height: 36px; border-radius: 8px; background: #D0520818; display: flex; align-items: center; justify-content: center; margin: 0 auto .5rem; color: var(--accent); }
    .ae-new-preview {
    ...
    .ae-social-row:focus-within { border-color: var(--accent); box-shadow: 0 0 0 3px #D0520818; }
    ...
    .ae-preview-banner { height: 56px; background: linear-gradient(135deg,#D0520828,#e4c99015); border-bottom: 1px solid var(--border); }
...
<div class="ae-card">
    ...
    <div class="ae-card-body">
        <div class="ae-row-2" style="grid-template-columns:1fr 1fr 1fr;">
            <div>
                <label class="ae-label">Province</label>
                <select name="province" id="provinceSelect" class="ae-select @error('province') is-invalid @enderror" onchange="onProvinceChange()">
                    <option value="">Select province…</option>
                    @foreach($rwProvinces as $province)
                        <option value="{{ $province }}" {{ old('province', $agent->province) == $province ? 'selected' : '' }}>{{ $province }}</option>
                    @endforeach
                </select>
                @error('province')<p class="ae-error">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="ae-label">District</label>
                <select name="district" id="districtSelect" class="ae-select @error('district') is-invalid @enderror" onchange="onDistrictChange()">
                    <option value="">Select district…</option>
                </select>
                @error('district')<p class="ae-error">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="ae-label">Sector</label>
                <select name="sector" id="sectorSelect" class="ae-select @error('sector') is-invalid @enderror">
                    <option value="">Select sector…</option>
                </select>
                @error('sector')<p class="ae-error">{{ $message }}</p>@enderror
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    const initialDistrict = @json(old('district', $agent->district));
    const initialSector   = @json(old('sector', $agent->sector));
    ...
    function onProvinceChange() {
        populateDistricts(document.getElementById('provinceSelect').value);
        document.getElementById('sectorSelect').innerHTML = '<option value="">Select sector…</option>';
    }

    function onDistrictChange() {
        populateSectors(document.getElementById('districtSelect').value);
    }

    document.addEventListener('DOMContentLoaded', () => {
        const province = document.getElementById('provinceSelect').value;
        if (province) {
            populateDistricts(province, initialDistrict);
            if (initialDistrict) populateSectors(initialDistrict, initialSector);
        }
    });
</script>
@endsection