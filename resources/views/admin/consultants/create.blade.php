    ...
    <form method="POST" action="{{ route('admin.consultants.store') }}" enctype="multipart/form-data">
        ...
        <div class="cc-layout">
            <div class="cc-main">
                ...
                <div class="cc-card">
                    <div class="cc-card-header">
                        <div class="cc-card-header-icon"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        ...
                        <div class="cc-gap">
                            <div class="cc-row-2">
                                <div>
                                    ...
                                    @error('name')<p class="cc-error"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                            <circle cx="12" cy="12" r="10" />
                                            <path d="M12 8v4m0 4h.01" />
                        ...
                        <div class="cc-cat-grid" id="catGrid">
                            ...
                            <label for="cat{{ $cat->id }}" class="cc-cat-label">
                                ...
                                <span class="cc-cat-count" id="count-{{ $cat->id }}"
                                    style="margin-left:auto;font-size:.68rem;background:var(--teal);color:#fff;
                                     border-radius:20px;padding:1px 7px;display:none">0</span>
                            </label>
                            @endforeach
                        </div>

                        {{-- Step 2: services panel (shown per selected category) --}}
            ...
            <div class="cc-side">
                ...
                <div class="cc-card">
                    <div class="cc-card-header">
                        <div class="cc-card-header-icon" style="background:#eff6ff;color:var(--blue);"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    ...
                    <div class="cc-card-body">
                        <div class="cc-toggle-row" style="padding-top:0;">
                            <div>
                                <div class="cc-toggle-label">Send welcome email</div>
                                <div class="cc-toggle-desc">Email login credentials on creation</div>
                            </div>
...
<script>
    ...
    function handleCategoryChange() {
        ...
        if (this.checked) {
            ...
            if (group) {
                ...
                // Uncheck all services in this group when category is deselected
                group.querySelectorAll('.service-check').forEach(s => s.checked = false);
            }
        }

        // Show/hide the entire services panel
    ...
    // Badge count per category button
    function updateCounts() {
        document.querySelectorAll('.cat-trigger').forEach(cb => {
            const catId = cb.dataset.catId;
            const count = document.querySelectorAll(
                `.service-check[data-group="${catId}"]:checked`
            ).length;
    ...
    // Restore state on page load (for old() values after validation failure)
