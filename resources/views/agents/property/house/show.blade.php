...
.hd-back-btn { display:inline-flex; align-items:center; gap:6px; padding:7px 14px; border:1.5px solid #e2e8f0; border-radius:8px; background:#fff; font-size:.81rem; font-weight:500; color:#475569; transition:all .18s; text-decoration:none; }
.hd-back-btn:hover { border-color:#3b82f6; color:#2563eb; background:#eff6ff; }
.hd-back-btn svg { width:14px; height:14px; }
.hd-action-btn { display:inline-flex; align-items:center; gap:6px; padding:7px 14px; border-radius:8px; font-size:.81rem; font-weight:600; border:none; cursor:pointer; transition:all .18s; text-decoration:none; }
...
/* ── Gallery ── */
.hd-gallery { display:grid; grid-template-columns:1fr 1fr; grid-template-rows:260px 170px; gap:8px; border-radius:12px; overflow:hidden; margin-bottom:24px; }
.hd-gal-main  { grid-row:1/3; position:relative; overflow:hidden; background:#f1f5f9; }
.hd-gal-thumb { position:relative; overflow:hidden; background:#f1f5f9; }
.hd-gal-main img, .hd-gal-thumb img { width:100%; height:100%; object-fit:cover; display:block; transition:transform .45s ease; }
.hd-gal-main:hover img, .hd-gal-thumb:hover img { transform:scale(1.04); }
.hd-gal-ph { width:100%; height:100%; display:flex; align-items:center; justify-content:center; background:linear-gradient(135deg,#f1f5f9,#e2e8f0); }
.hd-gal-ph svg { width:36px; height:36px; color:#cbd5e1; }
.hd-photo-actions { position:absolute; top:8px; right:8px; display:flex; gap:5px; z-index:3; }
.hd-photo-btn { width:30px; height:30px; border-radius:7px; background:rgba(255,255,255,.85); backdrop-filter:blur(6px); border:1px solid rgba(255,255,255,.5); display:grid; place-items:center; cursor:pointer; color:#475569; transition:all .18s; text-decoration:none; }
.hd-photo-btn:hover { background:#fff; color:#2563eb; }
.hd-photo-btn svg { width:13px; height:13px; }
.hd-photo-count { position:absolute; bottom:8px; left:8px; padding:2px 8px; border-radius:5px; background:rgba(0,0,0,.55); color:#fff; font-size:.68rem; font-weight:600; z-index:3; }

/* Upload zone */
.hd-upload-zone { border:2px dashed #e2e8f0; border-radius:10px; padding:20px; text-align:center; background:#fafbfc; cursor:pointer; transition:all .18s; }
.hd-upload-zone:hover, .hd-upload-zone.dragover { border-color:#3b82f6; background:#eff6ff; }
.hd-upload-zone svg { width:26px; height:26px; color:#94a3b8; margin-bottom:6px; }
.hd-upload-zone p { font-size:.8rem; color:#64748b; margin:0; }
.hd-upload-zone span { font-size:.72rem; color:#94a3b8; }
.hd-upload-previews { display:flex; flex-wrap:wrap; gap:8px; margin-top:10px; }
.hd-prev-thumb { position:relative; width:64px; height:52px; border-radius:7px; overflow:hidden; border:1px solid #e2e8f0; }
...
/* ── Specs strip ── */
.hd-specs { display:grid; grid-template-columns:repeat(auto-fit,minmax(100px,1fr)); gap:0; border:1px solid #e2e8f0; border-radius:10px; overflow:hidden; margin-bottom:20px; }
.hd-spec-cell { padding:13px 10px; text-align:center; border-right:1px solid #e2e8f0; }
.hd-spec-cell:last-child { border-right:none; }
.hd-spec-label { font-size:.67rem; color:#94a3b8; text-transform:uppercase; letter-spacing:.06em; margin-bottom:4px; }
.hd-spec-val { font-weight:700; font-size:.86rem; color:#1e293b; display:flex; align-items:center; justify-content:center; gap:4px; }
.hd-spec-val svg { width:12px; height:12px; color:#64748b; }

/* ── Cards ── */
.hd-card { background:#fff; border:1px solid #e5e7eb; border-radius:12px; margin-bottom:16px; overflow:hidden; }
.hd-card-head { padding:13px 18px; border-bottom:1px solid #f1f5f9; display:flex; align-items:center; justify-content:space-between; gap:10px; }
.hd-card-head-title { font-size:.88rem; font-weight:700; color:#1e293b; margin:0; display:flex; align-items:center; gap:6px; }
.hd-card-head-title svg { width:14px; height:14px; color:#64748b; }
...
/* ── Detail table ── */
.hd-detail-row { display:flex; align-items:flex-start; justify-content:space-between; padding:9px 0; border-bottom:1px solid #f8fafc; font-size:.82rem; }
.hd-detail-row:last-child { border-bottom:none; }
.hd-detail-label { color:#94a3b8; font-weight:500; flex-shrink:0; }
.hd-detail-val   { color:#1e293b; font-weight:600; text-align:right; }

/* ── Amenity grid ── */
.hd-amenity-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(160px,1fr)); gap:8px; }
.hd-amenity { display:flex; align-items:center; gap:10px; padding:10px 12px; border-radius:9px; background:#f8fafc; border:1px solid #e2e8f0; font-size:.8rem; color:#475569; transition:all .18s; }
.hd-amenity:hover { border-color:#bfdbfe; background:#eff6ff; color:#1d4ed8; }
.hd-amenity-icon { width:28px; height:28px; border-radius:7px; background:#eff6ff; border:1px solid #bfdbfe; display:grid; place-items:center; flex-shrink:0; }
.hd-amenity-icon svg { width:13px; height:13px; color:#2563eb; }

/* ── Sidebar ── */
.hd-sidebar { position:sticky; top:24px; }
.hd-sb-price { background:#0E0E0C; border-radius:var(--r,12px); overflow:hidden; margin-bottom:14px; position:relative; }
.hd-sb-price::before { content:''; position:absolute; inset:0; background:radial-gradient(ellipse 70% 60% at 20% 50%,rgba(37,99,235,.18) 0%,transparent 65%); pointer-events:none; }
...
.hd-cond-badge.for_sale { background:rgba(59,130,246,.15); border:1px solid rgba(59,130,246,.3); color:#93c5fd; }
...
/* Agent card */
.hd-agent-av { width:48px; height:48px; border-radius:50%; background:#eff6ff; border:2px solid #bfdbfe; display:grid; place-items:center; font-weight:700; font-size:1rem; color:#2563eb; flex-shrink:0; }
.hd-agent-row { display:flex; align-items:center; justify-content:space-between; padding:8px 0; border-bottom:1px solid #f8fafc; font-size:.8rem; }
.hd-agent-row:last-child { border-bottom:none; }

/* Plan card */
.hd-plan-item { display:flex; align-items:center; justify-content:space-between; padding:9px 0; border-bottom:1px solid #f1f5f9; font-size:.81rem; }
.hd-plan-item:last-child { border-bottom:none; }
.hd-plan-label { color:#94a3b8; }
.hd-plan-val   { font-weight:600; color:#1e293b; }

@media (max-width:640px) { .hd-gallery { grid-template-columns:1fr; grid-template-rows:200px 130px 130px; } .hd-gal-main { grid-row:auto; } }
</style>

{{-- ── Top bar ── --}}
<div class="hd-topbar">
    <div class="d-flex align-items-center gap-2">
        <a href="{{ route('agent.properties.houses.index') }}" class="hd-back-btn">
        ...
        <nav aria-label="breadcrumb" class="d-none d-md-block">
            <ol class="breadcrumb mb-0 small">
                ...
                <li class="breadcrumb-item"><a href="{{ route('agent.properties.houses.index') }}">Houses</a></li>
                <li class="breadcrumb-item active text-truncate" style="max-width:180px">{{ $house->title }}</li>
            </ol>
        </nav>
    </div>

    {{-- Action buttons — hidden for agents --}}
    @if(!$isAgent)
    <div class="d-flex align-items-center gap-2 flex-wrap">
        <a href="{{ route('agent.properties.houses.edit', $house->id) }}" class="hd-action-btn hd-btn-edit">
...
<div class="row g-3">
    ...
    <div class="col-xl-8">
        ...
        <div class="hd-card">
            <div class="hd-card-head">
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('agent.properties.houses.images.download', $house->id) }}"
                    ...
                    @endif
                    {{-- Upload button — hidden for agents --}}
                    @if(!$isAgent)
            ...
            <div class="hd-card-body">
                ...
                <div class="hd-gallery">
                    ...
                    <div class="hd-gal-main">
                        ...
                        <div class="hd-photo-actions">
                            <a href="{{ asset('storage/'.$house->images->first()->image_path) }}" download class="hd-photo-btn" title="Download">
                                <svg viewBox="0 0 24 24" fill="currentColor"><path d="M13 10h5l-6 6-6-6h5V3h2v7zm-9 9h16v2H4v-2z"/></svg>
                            </a>
                            <a href="{{ asset('storage/'.$house->images->first()->image_path) }}" target="_blank" class="hd-photo-btn" title="View full size">
                    ...
                    <div class="hd-gal-thumb">
                        ...
                        <img src="{{ asset('storage/'.$house->images->get(1)->image_path) }}" alt="{{ $house->title }}" loading="lazy">
                        <div class="hd-photo-actions">
                            <a href="{{ asset('storage/'.$house->images->get(1)->image_path) }}" download class="hd-photo-btn" title="Download">
                                <svg viewBox="0 0 24 24" fill="currentColor"><path d="M13 10h5l-6 6-6-6h5V3h2v7zm-9 9h16v2H4v-2z"/></svg>
                            </a>
                        </div>
                        @else
                    ...
                    <div class="hd-gal-thumb" style="position:relative">
                        ...
                        <img src="{{ asset('storage/'.$house->images->get(2)->image_path) }}" alt="{{ $house->title }}" loading="lazy">
                        <div class="hd-photo-actions">
                            <a href="{{ asset('storage/'.$house->images->get(2)->image_path) }}" download class="hd-photo-btn" title="Download">
                                <svg viewBox="0 0 24 24" fill="currentColor"><path d="M13 10h5l-6 6-6-6h5V3h2v7zm-9 9h16v2H4v-2z"/></svg>
                            </a>
                        </div>
                ...
                {{-- Upload zone — hidden entirely for agents --}}
                @if(!$isAgent)
                <div class="collapse" id="uploadZone">
                    <form method="POST" action="{{ route('agent.properties.houses.images.upload', $house->id) }}"
        ...
        <div class="hd-card">
            <div class="hd-card-head">
                <h6 class="hd-card-head-title"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6z"/></svg>Description</h6>
            </div>
            <div class="hd-card-body">
        ...
        <div class="hd-card">
            <div class="hd-card-head">
                <h6 class="hd-card-head-title"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg>Property Details</h6>
                {{-- Edit button — hidden for agents --}}
                @if(!$isAgent)
                <a href="{{ route('agent.properties.houses.edit', $house->id) }}"
                   class="btn btn-sm btn-outline-secondary d-flex align-items-center gap-1">
        ...
        <div class="hd-card">
            <div class="hd-card-head">
                <h6 class="hd-card-head-title"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>Amenities &amp; Features</h6>
            </div>
            <div class="hd-card-body">
                <div class="hd-amenity-grid">
                    ...
                    <div class="hd-amenity"><div class="hd-amenity-icon"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/></svg></div>{{ $house->city }}</div>
                    <div class="hd-amenity"><div class="hd-amenity-icon"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg></div>{{ $house->type ?? 'House' }}</div>
                    @foreach($house->facilities ?? [] as $facility)
                    <div class="hd-amenity"><div class="hd-amenity-icon"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg></div>{{ $facility->name }}</div>
                    @endforeach
                </div>
            </div>
        </div>
        ...
        <div class="hd-card">
            <div class="hd-card-head">
                <h6 class="hd-card-head-title"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/></svg>Location Map</h6>
            </div>
            <div style="padding:0">
    ...
    <div class="col-xl-4">
        <div class="hd-sidebar">
            ...
            {{-- Agent card ── --}}
            <div class="hd-card">
                <div class="hd-card-head"><h6 class="hd-card-head-title">Listed By</h6></div>
                <div class="hd-card-body">
            ...
            <div class="hd-card">
                ...
                <div class="hd-card-body">
                    ...
                    {{-- Approve button — hidden for agents --}}
                    @if($latestOrder->payment?->status === 'success' && !$house->is_approved && !$isAgent)
            ...
            {{-- Quick actions ── --}}
            <div class="hd-card">
                <div class="hd-card-head"><h6 class="hd-card-head-title">Quick Actions</h6></div>
                <div class="hd-card-body d-flex flex-column gap-2">

                    {{-- Modification actions — hidden for agents --}}
                    @if(!$isAgent)
                    <a href="{{ route('agent.properties.houses.edit', $house->id) }}"
                    ...
                    <button class="btn btn-outline-secondary btn-sm d-flex align-items-center gap-2"
                        ...
                        <svg viewBox="0 0 24 24" fill="currentColor" style="width:14px;height:14px"><path d="M14 13v4h-4v-4H7l5-5 5 5h-3z"/></svg>
                        Upload Photos
                    </button>
                    @endif

                    {{-- Read-only actions — visible to everyone --}}
                    @if($house->images->count())
                    <a href="{{ route('agent.properties.houses.images.download', $house->id) }}"
...
{{-- ══ APPROVE MODAL — hidden for agents ══ --}}
@if($latestOrder?->payment?->status === 'success' && !$house->is_approved && !$isAgent)
<div class="modal fade" id="approveModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" style="max-width:400px">
        <form method="POST" action="{{ route('agent.properties.houses.approve', $house) }}" class="modal-content">
            @csrf
...
{{-- ══ DELETE MODAL — hidden for agents ══ --}}
@if(!$isAgent)
    ...
    <div class="modal-dialog modal-dialog-centered" style="max-width:380px">
        <div class="modal-content p-4 text-center">
            ...
            <div class="d-flex justify-content-center gap-2">
                <form method="POST" action="{{ route('agent.properties.houses.destroy', $house->id) }}">
                    @csrf @method('DELETE')
...
{{-- Upload JS — only rendered when not an agent --}}
@if(!$isAgent)
<script>
(function () {
    ...
    if (input) input.addEventListener('change', e => {
        files = [...files, ...Array.from(e.target.files)]; renderPreviews();
    });
