        ...
        <div class="hd-sidebar">
            ...
            <div class="hd-card">
                ...
                <div class="hd-card-body">
                    ...
                    @if($payment->status === 'completed' && !$house->is_approved && !$isAgent)
                    ...
                    @endif
                    {{-- Complete payment link — hidden for agents (agents don't own the invoice) --}}
                    @if($payment->status === 'pending' && !$isAgent)
            ...
            <div class="hd-card">
                ...
                <div class="hd-card-body d-flex flex-column gap-2">
                    ...
                    <a href="{{ route('agent.properties.houses.edit', $house->id) }}" class="hd-outline-btn">
                    ...
                    @if($house->images->count())
                    <a href="{{ route('agent.properties.houses.images.download', $house->id) }}" class="hd-outline-btn">
                        <svg viewBox="0 0 24 24" fill="currentColor"><path d="M13 10h5l-6 6-6-6h5V3h2v7zm-9 9h16v2H4v-2z"/></svg>
                        Download All Photos
                    </a>
                    @endif
...
{{-- ══ STATUS MODAL — hidden for agents ══ --}}
@if(!$isAgent)
<div class="modal fade" id="statusModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" style="max-width:400px">
        <form method="POST" action="{{ route('agent.properties.houses.status', $house) }}" class="modal-content">
            @csrf @method('PATCH')
...
@if(isset($payment) && $payment?->status === 'completed' && !$house->is_approved && !$isAgent)
...
<script>
(function() {
    /* ── Gallery Lightbox — always available, even for agents (view-only) ── */
    ...
    /* ── Preview thumbnail strip — always available ── */
    ...
    /* ── Photo upload zone — only rendered/wired when not an agent ── */
