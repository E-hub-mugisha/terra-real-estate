...
.ah-page { background: var(--bg); min-height: 100vh; padding: 40px 0 80px; font-family: 'DM Sans', sans-serif; }

/* ── Page header ── */
.ah-page-header { margin-bottom: 36px; }
.ah-eyebrow {
    display: inline-flex; align-items: center; gap: 8px;
    font-size: .68rem; font-weight: 500; letter-spacing: .14em;
    text-transform: uppercase; color: var(--gold); margin-bottom: 8px;
}
.ah-eyebrow::before { content: ''; width: 18px; height: 1px; background: var(--gold); opacity: .5; }
.ah-page-header h1 {
    font-family: 'Cormorant Garamond', serif;
    font-size: clamp(1.6rem, 3vw, 2.4rem);
    font-weight: 500; letter-spacing: -.02em; color: var(--text); margin: 0;
}
.ah-page-header h1 em { font-style: italic; color: var(--gold); }
.ah-page-header p { font-size: .83rem; color: var(--muted); margin-top: 6px; }
...
.ah-errors ul { margin: 0; padding-left: 18px; }
.ah-errors li { font-size: .8rem; color: #dc2626; }

/* ── Layout ── */
.ah-layout { display: grid; grid-template-columns: 260px 1fr; gap: 28px; align-items: start; }
@media (max-width: 900px) { .ah-layout { grid-template-columns: 1fr; } }

/* ── Sidebar ── */
.ah-sidebar { position: sticky; top: 24px; }
.ah-step-nav { display: flex; flex-direction: column; gap: 0; }
.ah-nav-item {
...
.ah-nav-item.active:not(:last-child)::after { background: rgba(200,135,58,.3); }
.ah-nav-item.done:not(:last-child)::after   { background: rgba(200,135,58,.5); }

.ah-nav-circle {
    width: 28px; height: 28px; border-radius: 50%;
    display: grid; place-items: center; flex-shrink: 0;
    font-size: .72rem; font-weight: 700;
    border: 1.5px solid var(--border2); color: var(--dim);
...
.ah-nav-circle svg { width: 12px; height: 12px; }
.ah-nav-label { padding-top: 4px; }
.ah-nav-title { font-size: .8rem; font-weight: 600; color: var(--dim); transition: color var(--t); }
.ah-nav-item.active .ah-nav-title { color: var(--text); }
.ah-nav-item.done .ah-nav-title   { color: var(--muted); }
.ah-nav-sub { font-size: .7rem; color: var(--dim); margin-top: 1px; }
.ah-nav-item.active .ah-nav-sub { color: var(--gold); }

/* Progress */
.ah-progress-wrap { margin-bottom: 20px; }
.ah-progress-label {
    display: flex; justify-content: space-between;
    font-size: .72rem; color: var(--dim); margin-bottom: 6px;
}
.ah-progress-label strong { color: var(--text); }
.ah-progress-bar { height: 4px; background: var(--border); border-radius: 2px; overflow: hidden; }
.ah-progress-fill {
...
/* ── Steps ── */
.ah-form-area { min-width: 0; }
.ah-step { display: none; animation: ahIn .35s cubic-bezier(.4,0,.2,1) both; }
.ah-step.active { display: block; }
@keyframes ahIn  { from { opacity:0; transform:translateX(22px); } to { opacity:1; transform:translateX(0); } }
@keyframes ahBack{ from { opacity:0; transform:translateX(-22px); } to { opacity:1; transform:translateX(0); } }
.ah-step.going-back { animation: ahBack .35s cubic-bezier(.4,0,.2,1) both; }
...
.ah-panel-icon svg { width: 15px; height: 15px; color: var(--gold); }
.ah-panel-title {
    font-family: 'Cormorant Garamond', serif;
    font-size: 1.05rem; font-weight: 600; letter-spacing: -.01em; color: var(--text); margin: 0;
}
.ah-panel-body { padding: 20px 22px 24px; }

/* ── Step header ── */
.ah-step-header { margin-bottom: 20px; }
.ah-step-num { font-size: .7rem; font-weight: 600; letter-spacing: .1em; text-transform: uppercase; color: var(--gold); margin-bottom: 4px; }
.ah-step-header h2 {
    font-family: 'Cormorant Garamond', serif;
    font-size: 1.4rem; font-weight: 600; letter-spacing: -.01em; color: var(--text); margin: 0;
}
.ah-step-header p { font-size: .82rem; color: var(--muted); margin-top: 3px; }

/* ── Fields ── */
.ah-field { display: flex; flex-direction: column; gap: 5px; margin-bottom: 14px; }
.ah-field:last-child { margin-bottom: 0; }
.ah-field label {
    font-size: .72rem; font-weight: 600; text-transform: uppercase;
    letter-spacing: .06em; color: var(--muted);
}
.ah-field label .req { color: #dc2626; margin-left: 2px; }
.ah-field input,
.ah-field textarea,
.ah-field select {
    padding: 10px 13px;
...
.ah-field textarea::placeholder { color: var(--dim); }
...
.ah-field textarea { resize: vertical; min-height: 100px; }
.ah-field .hint { font-size: .71rem; color: var(--dim); }
...
.ah-file-zone:hover { border-color: var(--gold); background: var(--gold-bg); }
.ah-file-zone input { position: absolute; inset: 0; opacity: 0; cursor: pointer; width: 100%; height: 100%; }
.ah-file-zone svg { width: 26px; height: 26px; color: var(--dim); margin-bottom: 8px; }
.ah-file-zone .fz-title { font-size: .82rem; color: var(--muted); font-weight: 500; }
.ah-file-zone .fz-sub   { font-size: .72rem; color: var(--dim); margin-top: 3px; }
.fz-picked { display:none; font-size: .82rem; color: var(--green); font-weight: 500; margin-top: 6px; }
...
.img-preview-cell img { width: 100%; height: 100%; object-fit: cover; display: block; }

.ah-row   { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }
.ah-row-3 { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 14px; }
@media (max-width: 640px) { .ah-row, .ah-row-3 { grid-template-columns: 1fr; } }

/* UPI badge style input */
.ah-upi-wrap { position: relative; }
.ah-upi-wrap input { padding-left: 40px; font-family: 'DM Mono', monospace; letter-spacing: .06em; font-size: .82rem; }
.ah-upi-badge {
...
.ah-notice {
    ...
    background: var(--gold-bg); border: 1px solid var(--gold-bd);
    font-size: .8rem; color: var(--muted); margin-bottom: 16px; line-height: 1.6;
}
.ah-notice svg { width: 15px; height: 15px; color: var(--gold); flex-shrink: 0; margin-top: 1px; }

/* Zoning cards */
.zoning-grid {
    display: grid; grid-template-columns: repeat(auto-fill, minmax(145px,1fr)); gap: 8px;
}
.zoning-check { position: relative; }
.zoning-check input[type="radio"] { position: absolute; opacity: 0; width: 0; height: 0; }
.zoning-check label {
...
.zoning-check label .z-code {
    ...
    font-weight: 700; color: var(--gold); margin-bottom: 3px; letter-spacing: .04em;
}
.zoning-check label .z-desc { font-size: .72rem; color: var(--dim); margin-top: 2px; }
.zoning-check input:checked + label {
    border-color: var(--gold-bd); background: var(--gold-bg); color: var(--text);
}
.zoning-check input:checked + label .z-desc { color: var(--muted); }
.zoning-check label:hover { border-color: var(--gold-bd); background: var(--gold-bg); }

/* Nav buttons */
.ah-nav-btns { display: flex; align-items: center; gap: 10px; margin-top: 24px; }
.ah-btn-back {
...
.ah-btn-back:hover { border-color: var(--gold); color: var(--gold); }
.ah-btn-next {
    flex: 1; padding: 12px 22px; border-radius: 9px;
...
.ah-btn-next:hover { background: #a06828; transform: translateY(-1px); }
.ah-btn-next svg { width: 15px; height: 15px; }
</style>
                    ...
                    <div class="ah-panel">
                        ...
                        <div class="ah-panel-body">
                                ...
                                <div class="ah-field">
                                    ...
                                    <div class="ah-file-zone" style="padding:16px">
                                        ...
                                        <div class="fz-picked" id="doc-picked">
                                            <svg viewBox="0 0 24 24" fill="currentColor" style="width:14px;height:14px;color:var(--green);display:inline-block;margin-right:4px"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            ...
                            <div class="ah-notice" style="margin-top:8px">
                                <svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2a10 10 0 100 20A10 10 0 0012 2zm1 14H11v-4h2v4zm0-6H11V8h2v2z"/></svg>
...
(function () {
    ...
    function showStep(n, back) {
        ...
        if (n === TOTAL - 1) {
            ...
            btnNext.onclick = () => { if (validate(cur)) document.getElementById('ah-form').submit(); };
        } else {
    ...
    navItems.forEach((ni, i) => {
        ni.addEventListener('click', () => { if (i < cur) showStep(i, true); });
    });
