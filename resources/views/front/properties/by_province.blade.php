...
.pp-eyebrow::before { content: ''; width: 18px; height: 1px; background: var(--gold); opacity: .5; }
.pp-hero h1 {
...
.pp-hero p { font-size: .85rem; color: var(--muted); margin-top: 6px; }
...
.pp-stat.homes-stat { background: rgba(59,110,90,.07); border-color: rgba(59,110,90,.2); color: var(--green); }
.pp-stat.lands-stat { background: rgba(139,105,20,.07); border-color: rgba(139,105,20,.2); color: var(--amber); }
...
.pp-search input:focus { outline: none; border-color: var(--gold); background: var(--surface); }
...
/* Count + view toggle */
.pp-meta { display: flex; align-items: center; gap: 8px; margin-left: auto; }
...
.pp-vbtn.on, .pp-vbtn:hover { background: var(--gold); border-color: var(--gold); color: #fff; }
...
.pp-section-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
.pp-section-title {
    font-family: 'Cormorant Garamond', serif;
    font-size: 1.1rem; font-weight: 500; color: var(--text); margin: 0;
}
.pp-section-badge {
...
@keyframes ppFu { from { opacity:0; transform:translateY(14px); } to { opacity:1; transform:translateY(0); } }
...
.pp-card-foot {
    ...
    padding-top: 9px; border-top: 1px solid var(--border); margin-top: auto;
}
...
.pp-grid.list-v .col-md-6 { flex: 0 0 100%; max-width: 100%; }
.pp-grid.list-v .pp-card { flex-direction: row; max-height: 150px; }
.pp-grid.list-v .pp-card-img { width: 180px; min-width: 180px; aspect-ratio: unset; flex-shrink: 0; }
.pp-grid.list-v .pp-card-body { padding: 11px 14px; }
@media (max-width: 480px) {
    .pp-grid.list-v .pp-card-img { width: 120px; min-width: 120px; }
}
...
<div class="pp-filter">
    <div class="container">
        <div class="pp-filter-inner">
            ...
            <div class="pp-search">
                <svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
               ...
               data-created="{{ $home->created_at->timestamp ?? 0 }}">
                ...
                <div class="pp-card-img">
                    ...
                    @if($home->images->first())
                        <img src="{{ asset('storage/'.$home->images->first()->image_path) }}" alt="{{ $home->title }}" loading="lazy">
                    @else
               ...
               data-created="{{ $land->created_at->timestamp ?? 0 }}">
                ...
                <div class="pp-card-img">
                    ...
                    @if($land->images && $land->images->first())
                        <img src="{{ asset('storage/'.$land->images->first()->image_path) }}" alt="{{ $land->title }}" loading="lazy">
                    @else
