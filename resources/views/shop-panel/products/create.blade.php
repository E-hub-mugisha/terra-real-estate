...
<style>
    ...
    [data-h-scope="product-form"] .page-sub { color: var(--terra-muted); font-size: .82rem; margin-top: .1rem; }

    /* ---------- Form shell (shared look with edit/profile forms) ---------- */
...
<div data-h-scope="product-form">
    ...
    <div class="page-header">
        ...
        <div>
            ...
            <div class="page-sub">New listings are reviewed before they go live on your shop.</div>
        </div>
    </div>

    <form method="POST" action="{{ route('shop-panel.products.store') }}" enctype="multipart/form-data" class="card form-card">
        @csrf
        @include('shop-panel.products._form', ['product' => null, 'subcategories' => collect()])

        <div class="form-actions">
            <div class="form-actions-hint">
                <i class="bi bi-shield-check"></i> Your product will be sent for review after you create it.
            </div>
            <div class="d-flex gap-2">
                <a href="{{ route('shop-panel.products.index') }}" class="btn btn-terra-outline">Cancel</a>
                <button type="submit" class="btn btn-terra">
                    <i class="bi bi-check2"></i> Create Product
                </button>
            </div>
        </div>
    </form>
